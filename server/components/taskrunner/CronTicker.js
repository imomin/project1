...
function Task(id, cron, minTick, jsondata) {
  this.id = id
  this.cron = cron
  this.jsondata = jsondata
  this.nextTick = later.schedule(later.parse.cron(cron, false)).next(1, Date.now() + minTick).getTime()
  this.dead = false
}
...
function CronTicker(opts) {
  ...
  function tick() {
    ...
    while(!heap.empty()) {
      ...
      var newT = new Task(t.id, t.cron, minTick, t.jsondata)
      add(t.id, newT)
      //self.emit('task', t.id, t.nextTick, newT.nextTick, newT.jsondata );
      self.emit(t.id, t.nextTick, newT.nextTick, newT.jsondata );
    }
  }

  this.set = function(tid, tcron, jsondata) {
    tid = String(tid)
    var newT = new Task(tid, tcron, minTick, jsondata)
